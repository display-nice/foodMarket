...
function cards() {
    ...
    // Ниже находятся два варианта создания карточек через запросы к серверу.
    // Они запрашивают данные карточек из файла db.json , находящегося в корне проекта. Этот файл находится на эмулированном сервере.
    // Этот сервер (localhost:3000) создаётся npm-пакетом "json-server", команда терминала "npx json-server db.json".
    ...
    // А это набор данных для карточек для демонстрации и тестирования. Сервер для них не нужен,
    // достаточно просто раскомментировать.
    new MenuItem(
        "img/tabs/vegy.jpg",
        "vegy",
        'Меню "Фитнес"',
...
window.addEventListener('DOMContentLoaded', () => {
    // модальное окно откроется через 50 секунд после открытия страницы
    const modalTimerID = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_6__.openCallbackScreen)('.modal', modalTimerID), 50000);
